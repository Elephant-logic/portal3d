...
export class PlayerController {
  ...
  update(input, dt){
    ...
    this.inWater   = this.body.position.y < SEA + 0.5;
    this.onGround  = pen > -0.15 && this.body.position.y > SEA - 0.5;
    ...
    if(this.inWater){
      // Buoyancy — gentle upward force, stronger when deeper
      const sub = SEA - this.body.position.y;
      this.body.linearVelocity.y += sub * 1.5 * dt;
      // Water drag
      this.body.linearVelocity.scaleSelf(Math.pow(0.80, dt * 60));
      // Jump = swim up
      if(input.jump) this.body.linearVelocity.y += 5 * dt;
    }
  ...
  // Build view matrix directly from yaw+pitch — no cross products, no degenerate cases
  // Standard OpenGL: yaw=0 looks down -Z, pitch up = positive
  getViewMatrix(){
    ...
    // right   = ( cy,    0,   -sy         )
    // up      = ( sy*sp, cp,   cy*sp      )
    // forward = (-sy*cp, sp,  -cy*cp      )  (camera looks in this direction)
    // View matrix rows = [right, up, -forward], each row dotted with -eye for translation
    // Column-major layout: e[col*4 + row]
    ...
    e[ 2]=bx;  e[ 6]=by;  e[10]=bz;  e[14]=-(bx*eye.x + by*eye.y + bz*eye.z);
