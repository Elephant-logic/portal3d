  ...
  _build(px, pz, t){
    ...
    for(const {step:st,r0,r1} of RINGS){
      ...
      for(let j=-r1;j<r1;j+=st){
        ...
        // Sample — portal X,Y → engine X,Z. Height → engine Y.
        const h1=fbmTerrain(wx,   wz,   t), h2=fbmTerrain(wx+st,wz,   t);
        const h3=fbmTerrain(wx+st,wz+st,t), h4=fbmTerrain(wx,   wz+st,t);

        const avg  =(h1+h2+h3+h4)*0.25;
        const slope=(Math.abs(h1-h3)+Math.abs(h2-h4))/(st*2);
        const [cr,cg,cb]=fbmBiomeRGB(avg,slope,hb);

        // Normal: cross product of quad edges
        ...
        const push=(x,y,z)=>verts.push(x,y,z, nx/nl,ny/nl,nz/nl, cr,cg,cb,1.0);
        push(wx,   h1,wz   );
        push(wx+st,h2,wz   );
        push(wx+st,h3,wz+st);
        push(wx,   h4,wz+st);
    ...
    // Water — flat plane at SEA=2.2 over underwater cells only
    for(const {step:st,r0,r1} of RINGS){
      const ox=Math.floor(px/st)*st, oz=Math.floor(pz/st)*st;
      for(let i=-r1;i<r1;i+=st)
      for(let j=-r1;j<r1;j+=st){
        ...
        const h1=fbmTerrain(wx,   wz,   t), h2=fbmTerrain(wx+st,wz,   t);
        const h3=fbmTerrain(wx+st,wz+st,t), h4=fbmTerrain(wx,   wz+st,t);
        // Only draw water surface where terrain is below sea
        if(h1>=SEA&&h2>=SEA&&h3>=SEA&&h4>=SEA) continue;
        const depth=Math.max(0,SEA-(h1+h2+h3+h4)*0.25);
        const alpha=Math.min(0.85,0.45+depth*0.08);
        const wr=isChaos?0.85:0.05, wg=isChaos?0.15:0.3, wb=isChaos?0.0:0.72;
        const pw=(x,z)=>verts.push(x,SEA,z, 0,1,0, wr,wg,wb,alpha);
        pw(wx,   wz); pw(wx+st,wz); pw(wx+st,wz+st); pw(wx,wz+st);
        inds.push(vi,vi+1,vi+2, vi,vi+2,vi+3);
        vi+=4;
      }
    }
