  ...
  _build(px,pz,t){
    ...
    for(const {step:st,r0,r1} of RINGS){
      ...
      for(let j=-r1; j<r1; j+=st){
        ...
        if(d<r0||d>=r1) continue;
        const wx=ox+i, wz=oz+j;
        // Only draw water where ALL corners are underwater
        if(fbmTerrain(wx,    wz,    t)>=SEA) continue;
        if(fbmTerrain(wx+st, wz,    t)>=SEA) continue;
        if(fbmTerrain(wx+st, wz+st, t)>=SEA) continue;
        if(fbmTerrain(wx,    wz+st, t)>=SEA) continue;
        const depth=Math.min(1,(SEA-fbmTerrain(wx,wz,t))/5);
