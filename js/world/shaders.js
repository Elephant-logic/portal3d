...
void main(){
  ...
  // Diffuse: clamp both by sun height (L.y) AND face angle
  // This ensures land is dark when sun is below horizon regardless of normal tilt
  float sunAbove = max(0.0, L.y);                    // 0 when sun below horizon
  float NdotL    = max(dot(N, L), 0.0) * sunAbove * (1.0/max(0.001, sunAbove));
  // Simplified: just use both clamps multiplied
